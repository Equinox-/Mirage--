...
Inventory::Inventory(int inventorySlots, Raknet& raknet, Map& map, PlayerInfo& playerInfo, Character& character)
:mInventorySlots(inventorySlots)
,mRaknet(raknet)
,mMap(map)
,mDragSprite(SVector2(1300.0f, 290.0f), false)
...
void Inventory::Update(float deltaTime)
{
	...
	// Check for item use
	if( ((x > dragSpriteX) && x < dragSpriteX + width) && (y > dragSpriteY && y < dragSpriteY + height) && mDragSprite.GetVisible())
	{
		...
		if(x > 27 && x < 70) xSlot = 0;
		if(x > 78 && x < 120) xSlot = 1;
		if(x > 127 && x < 170) xSlot = 2;
		if(x > 178 && x < 218) xSlot = 3;
		if(x > 226 && x < 270) xSlot = 4;

		int ySlot = -1;

		if(y > -4 && y < 34) ySlot = 0;
		if(y > 46 && y < 85) ySlot = 1;
		if(y > 93 && y < 135) ySlot = 2;
		if(y > 143 && y < 183) ySlot = 3;
		if(y > 192 && y < 234) ySlot = 4;
		if(y > 242 && y < 282) ySlot = 5;
...
void SetValues(ItemBase* item, char* itemName, int value, bool questItem, int imageNumber, char* description, int itemNumber)
{
	...
	case ID_PICKUP_ITEM:
		{
			...
			SetValues(item, itemName, value, questItem, imageNumber, description, itemNumber);
			item->GetData(bsIn);
			AddItem(item);
			break;
		}
...
void Inventory::Render()
{
	if(mDragSprite.GetVisible())
	{
		...
		for (std::vector<ItemBase*>::iterator iter = mItems.begin(); iter != mItems.end(); ++iter)
		{
